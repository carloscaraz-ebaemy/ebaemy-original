...
class AddWhatsappApiFieldsToConfigurationEcommerce extends Migration
{
    public function up()
    {
        Schema::connection('tenant')->table('configuration_ecommerce', function (Blueprint $table) {
            if (!Schema::connection('tenant')->hasColumn('configuration_ecommerce', 'whatsapp_api_token')) {
                $table->text('whatsapp_api_token')->nullable()->after('phone_whatsapp')
                      ->comment('Meta Cloud API bearer token (system-user token)');
            }
            if (!Schema::connection('tenant')->hasColumn('configuration_ecommerce', 'whatsapp_phone_id')) {
                $table->string('whatsapp_phone_id')->nullable()->after('whatsapp_api_token')
                      ->comment('Meta Phone Number ID from developer console');
            }
            if (!Schema::connection('tenant')->hasColumn('configuration_ecommerce', 'whatsapp_vendor_number')) {
