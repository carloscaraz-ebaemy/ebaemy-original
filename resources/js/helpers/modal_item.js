 ...
 */function ItemSlotTooltip(item) {
    ...
    if (item !== undefined
        && item.lots_group !== undefined
        && item.lots_group.length > 0
    ) {
        data += "Lotes: " + item.lots_group.length + ' lotes ' + "<br>";
        
    }

    if (item !== undefined
        && item.date_of_due !== undefined
        && item.date_of_due !== null 
    ) {
        data += "Fecha de vencimiento: " + item.date_of_due + "<br>";
