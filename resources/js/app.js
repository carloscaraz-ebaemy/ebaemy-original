...
Vue.component('tenant-ecommerce-configuration-color', require('../../modules/Ecommerce/Resources/assets/js/views/configuration_color/index.vue'));
...
Vue.component('x-input-service-guest', require('../../modules/ApiPeruDev/Resources/assets/js/components/InputServiceGuest.vue'));
...
Vue.component('system-email-configuration', require('./views/system/configuration/emailConfiguration.vue'))
// Configuración del login
Vue.component('tenant-login-page', require('./views/tenant/login/index.vue'))

// Agregar el componente de registro de invitados
Vue.component('system-guest-register-register', require('./views/system/guest-register/register.vue'));
Vue.component('system-guest-register-disabled', require('./views/system/guest-register/disabled.vue'));
...
Vue.mixin({
 ...
 //directiva agregada para issue #93 añadir icono cuando no hay datos

  Vue.directive('empty', {
    bind(el, binding, vnode) {

      const placeholder = document.createElement('div');
      placeholder.className = 'empty-state-wrapper';
      placeholder.style.display = 'none';
      el.__emptyPlaceholder = placeholder;

      const EmptyConstructor = Vue.extend({
        render(h) {
          return h('empty-state', {
            props: { 
              message: binding.arg || 'No hay datos disponibles' 
            }
          });
        }
      });
      
      el.__emptyInstance = new EmptyConstructor().$mount();
      placeholder.appendChild(el.__emptyInstance.$el);
      
      el.parentNode.insertBefore(placeholder, el);
      
      el.__originalDisplay = window.getComputedStyle(el).display;
    },
    ...
    update(el, binding) {
      ...
      if (isEmpty) {
        ...
        el.__emptyPlaceholder.style.display = 'block';

        if (binding.arg && el.__emptyInstance) {
          el.__emptyInstance.message = binding.arg;
        }
      } else {
        el.style.display = el.__originalDisplay || '';
        el.__emptyPlaceholder.style.display = 'none';
      }
    },

    unbind(el) {
      if (el.__emptyInstance) {
        el.__emptyInstance.$destroy();
      }
      if (el.__emptyPlaceholder && el.__emptyPlaceholder.parentNode) {
        el.__emptyPlaceholder.parentNode.removeChild(el.__emptyPlaceholder);
      }
    }
  });
