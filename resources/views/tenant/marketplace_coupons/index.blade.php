...
@section('content')
<div class="container-fluid" id="mkt-coupons-tenant" style="padding:24px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;flex-wrap:wrap;gap:10px">
        <div>
            <h3 style="margin:0">Cupones del marketplace</h3>
            <p style="margin:4px 0 0;color:#64748b;font-size:13.5px">Cupones que aplican SOLO en tu tienda, asignables a clientes registrados de ebaemy.com.</p>
        </div>
    ...
    <div id="mkt-coupon-form" style="display:none;background:#fff;border:1px solid #e5e7eb;border-radius:10px;padding:18px;margin-bottom:18px">
        ...
        <form onsubmit="return mktCreateCoupon(event)">
            <div class="row" style="gap:10px 0">
                <div class="col-md-4">
                    <label>Codigo</label>
                    <input name="code" required maxlength="40" class="form-control" placeholder="MITIENDA10">
                </div>
                <div class="col-md-8">
                    <label>Nombre interno</label>
                    <input name="name" required maxlength="100" class="form-control" placeholder="10% bienvenida cliente nuevo">
                </div>
                <div class="col-md-3">
                    <label>Tipo</label>
                ...
                <div class="col-md-3">
                    <label>Valor</label>
                    <input name="value" type="number" step="0.01" min="0.01" required class="form-control" placeholder="10">
                </div>
                <div class="col-md-3">
                    <label>Subtotal min (opc)</label>
                    <input name="min_subtotal" type="number" step="0.01" min="0" class="form-control" placeholder="50">
                </div>
                <div class="col-md-3">
    ...
    <table class="table" style="background:#fff">
        <thead><tr>
            <th>Codigo</th><th>Nombre</th><th>Tipo / Valor</th><th>Min</th>
    ...
    <div id="mkt-assign-modal" style="display:none;position:fixed;inset:0;background:rgba(15,23,42,.5);z-index:9999;align-items:center;justify-content:center">
        <div style="background:#fff;border-radius:12px;padding:24px;max-width:500px;width:90%">
            ...
            <p style="font-size:13.5px;color:#64748b">Pega los emails de tus clientes (uno por linea o separados por coma).</p>
            <form onsubmit="return mktAssign(event)">
                <input type="hidden" name="coupon_id" id="mkt-assign-cid">
                <textarea name="emails" rows="6" required class="form-control" placeholder="cliente1@email.com&#10;cliente2@email.com"></textarea>
                <label style="margin-top:10px">Vence el (opc)</label>
...
<script>
const recordsUrl = @json(route('tenant.marketplace_coupons.records'));
const storeUrl   = @json(route('tenant.marketplace_coupons.store'));
const csrf = @json(csrf_token());

function load() {
    fetch(recordsUrl).then(r => r.json()).then(j => {
        const tbody = document.getElementById('mkt-coupon-list');
        if (!j.data.length) { tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;color:#94a3b8;padding:24px">Aun no tienes cupones. Crea uno para empezar.</td></tr>'; return; }
        tbody.innerHTML = j.data.map(c => `
            <tr>
                <td><strong>${c.code}</strong></td>
                <td>${c.name}</td>
                <td>${c.type === 'percent' ? c.value+'%' : 'S/ '+c.value}</td>
                <td>${c.min_subtotal ? 'S/ '+c.min_subtotal : '—'}</td>
                <td>${c.valid_until || '—'}</td>
...
function toggleC(id) {
    fetch(`/marketplace/coupons/${id}/toggle`, {
...
function mktAssign(e) {
    ...
    fetch(`/marketplace/coupons/${id}/assign`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' },
